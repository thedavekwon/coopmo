...
docker run --name coopmoMysql --network coopmoNetwork --health-cmd='mysqladmin ping --silent' \
...
docker run --name coopmo --network coopmoNetwork -p 8080:8080 -t thedavekwon/coopmo